...
def main(file_name: str):
    ...
    N_VARIANTS = 3  # poseidon.nosig, poseidon, poseidon.nosig.bls
    N_RUNS = 30
    (n_compilation_runs, n_normal_runs) = check_parameters(log)
    if n_compilation_runs != N_VARIANTS * 1:
        err(f"expected {N_VARIANTS} * {N_RUNS} runs, found {n_compilation_runs}")
    if n_normal_runs != N_VARIANTS * N_RUNS:
        err(f"expected {N_VARIANTS} * {N_RUNS} runs, found {n_normal_runs}")
    ...
    def check_result_count(name, results, n):
    ...
    # Check: we should have one time for each step.
    for (program, variant, run) in compilation:
        if program in ["flexibility"]:
            check_result_count("compilation", compilation, 1)
            check_result_count("setup", setup, 1)
            check_result_count("compute_witness", compute_witness, 1)
            check_result_count("generate_proof", generate_proof, 1)
            check_result_count("aggregate_signatures", aggregate_signatures, 1)
            check_result_count("verify_total", verify_total, 1)
        else:
            err("unexpected program {program}")
