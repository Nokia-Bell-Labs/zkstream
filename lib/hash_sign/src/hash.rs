...
pub fn sha256_salted<M: SerializableMessage>(message: &M, salt: &SaltSHA256) -> Vec<u8> {
    let hash = Sha256::digest(&message.to_bytes_salted(salt));
    // eprintln!("hash: {:?}", hash);
    hash.to_vec()
}
...
pub fn sha256_unsalted<M: SerializableMessage>(message: &M) -> Vec<u8> {
    let hash = Sha256::digest(&message.to_bytes());
    // eprintln!("hash: {:?}", hash);
    hash.to_vec()
}
...
pub fn poseidon_salted<M: SerializableMessage>(message: &M, salt: &SaltPoseidon) -> Fr {
    let hash = Poseidon::new()
        .hash(message.to_fields_salted(salt))
        .unwrap();
...
pub fn poseidon_unsalted<M: SerializableMessage>(message: &M) -> Fr {
    let hash = Poseidon::new().hash(message.to_fields()).unwrap();
    // eprintln!("hash: {}", hash_fr);
    hash
}
...
mod tests {
    ...
    fn test_sha256_salted() {
        ...
        assert_eq!(
            ...
            sha256_salted(&MESSAGE.clone(), &SALT)
        );
    }
    ...
    fn test_poseidon_salted() {
        ...
        assert_eq!(
            ...
            .unwrap(),
            poseidon_salted(&MESSAGE.clone(), &salt_fields)
        );
    }
    ...
    fn test_poseidon_unsalted() {
        ...
        assert_eq!(
            ...
            .unwrap(),
            poseidon_unsalted(&MESSAGE.clone())
        );
    }
}
